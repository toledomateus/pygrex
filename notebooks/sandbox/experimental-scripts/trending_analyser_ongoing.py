...
def calculate_user_trending_score(
    ...
    for item_id, group in movie_ratings_per_month.groupby("itemId"):
        ...
        if (
            ...
            # All ratings are same, or only one rating
...
if __name__ == "__main__":
    ...
    param_peak_norm_min_height = (
        0.3  # Peak must be at least 30% of the movie's normalized range
    )
    param_peak_min_distance = (
        9  # Peaks must be at least 9 months apart (same as before, absolute)
    )
    param_peak_norm_min_prominence = (
        0.2  # Peak must stand out by 20% of normalized range
    )
    param_peak_width_rel_height = (
        0.6  # Measure width at 60% of normalized prominence-relative height
    )
    ...
    users_to_test.append(169321)
    users_to_test.append(138121)
