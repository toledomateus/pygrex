...
def scale_predictions(
    raw_predictions, target_min=1, target_max=5, ref_min=0, ref_max=6, method="linear"
):
    """
...
def generate_recommendation(model, user_id_str, movie_ids_to_pred, data):
    ...
    # Use the model's predict method to get raw scores for all unseen movies
    raw_predictions = model.predict(new_user_id, movie_ids_to_pred)
    ...
    # plt.title('Gaussian Plot for Scaled Predictions')
...
data.dataset = data.dataset.iloc[1:].reset_index(drop=True)

movie_ids = data.dataset["itemId"].unique()

# Train the recommendation model
algo = EMFModel(**cfg.model.emf)
algo.fit(data)
...
for group in all_groups:
    ...
    while True:
        ...
        # Retrain the recommendation model
        algo_retrained = EMFModel(**cfg.model.emf)
