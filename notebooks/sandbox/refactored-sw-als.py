...
# from pygrex.config import cfg # Assuming cfg is loaded elsewhere
...
class CfgObject:  # Placeholder
    def __init__(self):
        self.data = type(
            "obj", (object,), {"test": {"filepath_or_buffer": "dummy_path"}}
        )()
...
class DataReader:  # Placeholder definition
    def __init__(
        ...
        **kwargs,
        ...
        if dataframe is not None:
            ...
            print(
                f"Warning: Placeholder DataReader initialized without real data loading from {filepath_or_buffer}"
            )
        ...
        self.user_map_internal_to_original = {
            i: i for i in self.dataset["userId"].unique()
        }
        self.user_map_original_to_internal = {
            v: k for k, v in self.user_map_internal_to_original.items()
        }
        self.item_map_internal_to_original = {
            i: i for i in self.dataset["itemId"].unique()
        }
        self.item_map_original_to_internal = {
            v: k for k, v in self.item_map_internal_to_original.items()
        }
    ...
    def binarize(self, binary_threshold=1):
        ...
        if "rating" in self.dataset.columns:
            self.dataset["rating"] = (
                self.dataset["rating"] >= binary_threshold
            ).astype(int)

    def get_new_user_id(
        self, original_user_id: Union[int, np.integer, str]
    ) -> Optional[int]:
        """Gets the internal user ID for an original user ID."""
        original_user_id = (
            int(original_user_id)
            if isinstance(original_user_id, (int, np.integer))
            else original_user_id
        )
        return self.user_map_original_to_internal.get(original_user_id)

    def get_new_item_id(
        self, original_item_id: Union[int, np.integer, str]
    ) -> Optional[int]:
        """Gets the internal item ID for an original item ID."""
        original_item_id = (
            int(original_item_id)
            if isinstance(original_item_id, (int, np.integer))
            else original_item_id
        )
    ...
    def get_original_item_id(
        self, internal_item_id: Union[int, np.ndarray]
        ...
        if isinstance(internal_item_id, (int, np.integer)):
            ...
            return [
                self.item_map_internal_to_original.get(int(i))
                for i in internal_item_id
                if int(i) in self.item_map_internal_to_original
            ]
        return None  # Or raise error

    def get_new_user_ids(
        self, user_ids: List[Union[int, np.integer, str]]
    ) -> List[int]:
        """Converts a list of original user IDs to internal IDs."""
        return [
            self.get_new_user_id(uid)
            for uid in user_ids
            if self.get_new_user_id(uid) is not None
        ]

    def get_new_item_ids(
        self, item_ids: List[Union[int, np.integer, str]]
    ) -> List[int]:
        """Converts a list of original item IDs to internal IDs."""
        return [
            self.get_new_item_id(iid)
            for iid in item_ids
            if self.get_new_item_id(iid) is not None
        ]

    def get_original_item_ids(
        self, internal_item_ids: List[int]
    ) -> List[Union[int, str]]:
        """Gets the original item IDs from a list of internal IDs."""
        return [
            self.get_original_item_id(iid)
            for iid in internal_item_ids
            if self.get_original_item_id(iid) is not None
        ]

    def remove_interactions(
        self, user_ids: List[Union[int, str]], item_ids: List[Union[int, str]]
    ) -> pd.DataFrame:
        ...
        mask_to_drop = self.dataset["userId"].isin(internal_user_ids) & self.dataset[
            "itemId"
        ].isin(internal_item_ids)
...
class ALS:  # Placeholder definition
    ...
    def fit(self, data_reader: DataReader):
        ...
        self.user_map = (
            data_reader.user_map_original_to_internal
        )  # Store mapping used during training
        self.item_map = (
            data_reader.item_map_original_to_internal
        )  # Store mapping used during training
        ...
        print(
            f"Placeholder ALS: Fit complete. Users={self.num_users}, Items={self.num_items}"
        )
    ...
    def predict(self, internal_user_id: int, internal_item_id: int) -> Optional[float]:
        ...
        if self.user_factors is None or self.item_factors is None:
            ...
            return None
        if (
            internal_user_id >= self.num_users
            or internal_item_id >= self.num_items
            or internal_user_id < 0
            or internal_item_id < 0
        ):
        ...
        prediction = np.dot(
            self.user_factors[internal_user_id, :],
            self.item_factors[internal_item_id, :],
        )
...
DEFAULT_RAW_PREDICTION_MAX = (
    6.0  # Default reference max for scaling (adjust if ALS outputs different range)
)
...
def get_rated_items_by_group_members(
    group_member_ids: List[int], data_reader: DataReader
) -> List[Union[int, str]]:
    """
    ...
    original_item_ids = data_reader.get_original_item_ids(
        list(rated_internal_items)
    )  # Use list conversion
...
def get_items_for_recommendation(
    ...
    if not internal_group_ids:
        return (
            all_item_ids  # If group is empty or not in data, all items are candidates
        )
    ...
    items_to_predict_for = np.setdiff1d(
        all_item_ids, items_rated_by_group, assume_unique=True
    )
...
def scale_predictions(
    ...
    if method == "linear":
        ...
        if max_raw == min_raw:
            ...
            scaled_predictions = np.full_like(
                raw_predictions, (target_min + target_max) / 2
            )
        else:
...
def generate_recommendations_for_user(
    ...
    if internal_user_id is None:
        print(
            f"Warning: User {user_id_original} not found in data reader mapping for prediction."
        )
        return {}
    ...
    scaled_predictions = scale_predictions(
        raw_predictions_np, method="linear"
    )  # Or "quantile"
    ...
    if len(original_item_ids) != len(scaled_predictions):
        print(
            f"Warning: Mismatch between original item IDs ({len(original_item_ids)}) and scaled predictions ({len(scaled_predictions)}) for user {user_id_original}."
        )
...
def get_group_recommendations(
    ...
    if aggregation_strategy == "average":
        num_members = len(
            member_predictions
        )  # Use actual number of members with predictions
        if num_members == 0:
            return [] if top_n != 1 else None
        ...
        raise NotImplementedError(
            f"Aggregation strategy '{aggregation_strategy}' not implemented."
        )
...
def calculate_user_intensity(
    ...
    internal_group_ids = data_reader.get_new_user_ids(group_member_ids)
    internal_expl_item_ids = data_reader.get_new_item_ids(explanation_item_ids)
    explanation_size = len(
        explanation_item_ids
    )  # Use original list size for denominator
...
def calculate_popularity_mask(
    item_ids: List[Union[int, str]], data_reader: DataReader
) -> Dict[Union[int, str], float]:
    """
    ...
    popularity_mask = {}
    raw_popularity = {}
    internal_item_ids = data_reader.get_new_item_ids(
        item_ids
    )  # Get internal IDs for lookup
    interactions = data_reader.dataset
    ...
    item_counts = interactions[interactions["itemId"].isin(internal_item_ids)][
        "itemId"
    ].value_counts()
...
def calculate_relevance_mask(
    ...
    for user_id, predictions in member_predictions.items():
        relevance_mask[user_id] = predictions.get(
            target_item_id, 0.0
...
def calculate_item_intensity_score(
    item_id: Union[int, str], group_member_ids: List[int], data_reader: DataReader
) -> float:
    """
...
def build_explanation_candidate_chart(
    ...
    for item_id in items_rated_by_group:
        ...
        item_intensity_score = calculate_item_intensity_score(
            item_id, group_member_ids, data_reader
        )
        avg_rating_score = calculate_average_rating_score(
            item_id, group_member_ids, data_reader
        )
        relevance_score = calculate_item_relevance_score(
            item_id, group_member_ids, relevance_mask, data_reader
        )

        # Combine scores (simple summation, weights could be added)
        total_score = (
            pop_score + relevance_score + item_intensity_score + avg_rating_score
        )
...
def run_explanation_experiment():
    ...
    for group_index, group_string in enumerate(all_groups_strings):
        ...
        candidate_internal_items = get_items_for_recommendation(
            all_internal_item_ids, member_ids, data
        )
        ...
        for member_id in member_ids:
            user_predictions = generate_recommendations_for_user(
                algo,
                member_id,
                candidate_internal_items,
                data,  # Use initial algo and data
            )
        ...
        original_target_item = get_group_recommendations(
            initial_member_predictions, top_n=1
        )

        if original_target_item is None:
            print(
                "Could not determine initial target recommendation for the group. Skipping."
            )
            continue
        print(
            f"Initial Top-1 Group Recommendation (Target Item): {original_target_item}"
        )

        # --- Prepare for Explanation Search ---
        # Get items rated by group (original IDs)
        items_rated_by_group_original = get_rated_items_by_group_members(
            member_ids, data
        )
        if not items_rated_by_group_original:
            print(
                "Group members have not rated any items in the dataset. Cannot find explanation. Skipping."
            )
            continue
        ...
        relevance_mask = calculate_relevance_mask(
            original_target_item, initial_member_predictions
        )
        ...
        if not explanation_candidate_chart:
            print(
                "Could not build explanation candidate chart (no rated items found?). Skipping."
            )
            continue
        ...
        recommender_calls_count = (
            0  # Count how many times the *group* recommender is called after retraining
        )
        ...
        while recommender_calls_count < MAX_RECOMMENDER_CALLS:
            ...
            if window_items is None:
                print(
                    "Sliding window finished. No explanation found within call limit."
                )
            ...
            try:
                ...
                data_retrained_window = DataReader(
                    dataframe=changed_df_window
                )  # Use placeholder
                ...
                if data_retrained_window.dataset.empty:
                    print(
                        "  Warning: Dataset became empty after removing window items. Skipping window."
                    )
                    continue

                algo_retrained_window = ALS(
                    factors=ALS_FACTORS,
                    regularization=ALS_REGULARIZATION,
                    iterations=ALS_ITERATIONS,
                )  # Placeholder
                ...
                candidate_items_for_retrained = get_items_for_recommendation(
                    data_retrained_window.dataset[
                        "itemId"
                    ].unique(),  # Use items present in retrained data
                    member_ids,
                ...
                for member_id in member_ids:
                    user_preds_retrained = generate_recommendations_for_user(
                        algo_retrained_window,
                        member_id,
                        candidate_items_for_retrained,
                        data_retrained_window,
                    )
                    retrained_member_predictions_window[member_id] = (
                        user_preds_retrained
                    )
                ...
                print(
                    f"  New Top-{RECOMMENDATION_LIST_SIZE} Recs (after removing window): {group_rec_list_window}"
                )

            except Exception as e:
                print(
                    f"  Error during retraining or prediction for window {window_items}: {e}"
                )
            ...
            if original_target_item not in group_rec_list_window:
                print(
                    f"  Target item {original_target_item} removed! Searching minimal subset in window..."
                )
                ...
                for subset_size in range(1, len(window_items) + 1):
                    if minimal_explanation_found_in_window:
                        break  # Stop if minimal found

                    for explanation_candidate in itertools.combinations(
                        window_items, subset_size
                    ):
                        ...
                        if (
                            recommender_calls_count + checked_subsets_count
                            > MAX_RECOMMENDER_CALLS
                        ):
                            print("  Max call limit reached during subset check.")
                            minimal_explanation_found_in_window = (
                                True  # Break outer loops
                            )
                            break

                        print(
                            f"    [Sub-check {checked_subsets_count}] Testing subset: {explanation_candidate}"
                        )

                        # --- Retrain model excluding subset ---
                        # print(f"      Retraining model (excluding subset)...") # Verbose
                        try:
                            changed_df_subset = change_data(
                                data, member_ids, explanation_candidate
                            )
                            data_retrained_subset = DataReader(
                                dataframe=changed_df_subset
                            )  # Placeholder
                            ...
                            if data_retrained_subset.dataset.empty:
                                print(
                                    "      Warning: Dataset became empty after removing subset. Skipping subset."
                                )
                                continue

                            algo_retrained_subset = ALS(
                                factors=ALS_FACTORS,
                                regularization=ALS_REGULARIZATION,
                                iterations=ALS_ITERATIONS,
                            )  # Placeholder
                            ...
                            for member_id in member_ids:
                                user_preds_subset = generate_recommendations_for_user(
                                    algo_retrained_subset,
                                    member_id,
                                    candidate_items_for_subset,
                                    data_retrained_subset,
                                )
                                retrained_member_predictions_subset[member_id] = (
                                    user_preds_subset
                                )
                            ...
                            print(
                                f"      Error during retraining/prediction for subset {explanation_candidate}: {e}"
                            )
                        ...
                        if original_target_item not in group_rec_list_subset:
                            print("-" * 20)
                            print(
                                f"Minimal Counterfactual Explanation Found for Group {group_string}!"
                            )
                            print(
                                f"Explanation (Items to remove): {explanation_candidate}"
                            )
                            print(f"Original Target Item: {original_target_item}")
                            new_top_item = (
                                group_rec_list_subset[0]
                                if group_rec_list_subset
                                else "None"
                            )
                            print(
                                f"New Top Item (after removing explanation): {new_top_item}"
                            )
                            print(
                                f"Found at total recommender calls: {recommender_calls_count} (window) + {checked_subsets_count} (subset checks)"
                            )
                            print(f"Explanation Size: {len(explanation_candidate)}")

                            # Calculate metrics for the found explanation
                            avg_item_intensity = calculate_average_item_intensity(
                                explanation_candidate, member_ids, data
                            )
                            user_intensity = calculate_user_intensity(
                                explanation_candidate, member_ids, data
                            )
                            print(
                                f"Average Item Intensity per item in explanation: {[f'{x:.3f}' for x in avg_item_intensity]}"
                            )
                            print(
                                f"User Intensity per user in group: {[f'{x:.3f}' for x in user_intensity]}"
                            )
                            print("-" * 20)
        ...
        if not found_explanation:
            print(
                f"Could not find counterfactual explanation for group {group_string} within {MAX_RECOMMENDER_CALLS} calls."
            )
